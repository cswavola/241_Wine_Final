...
boxplot(d$Guess_the_Cost,horizontal =TRUE)
...
```{r}
DT<-data.table(d)
lm.des<-lm(Guess_the_Cost~Long_Description,DT)
summary(lm.des)
```
...
DT_rlm$French_Purchasing_Page<-factor(DT_rlm$French_Purchasing_Page)
DT_rlm$German_Purchasing_Page<-factor(DT_rlm$German_Purchasing_Page)
DT_rlm$US_Origin <- factor(DT_rlm$US_Origin)
DT_rlm$Long_Description <- factor(DT_rlm$Long_Description)

rlm.format<-rlm(Guess_the_Cost~French_Purchasing_Page + German_Purchasing_Page,DT_rlm)

rlm.origin<-rlm(Guess_the_Cost~US_Origin,DT_rlm)

rlm.description<-rlm(Guess_the_Cost~Long_Description,DT_rlm)

rlm.combined<-rlm(Guess_the_Cost~French_Purchasing_Page + German_Purchasing_Page + US_Origin + Long_Description,DT_rlm)


stargazer(rlm.combined,rlm.format,rlm.origin,rlm.description,main="RLM Basic regressions- no covariates", align=TRUE, type="text")
...
DT_rlm$US_Origin <- factor(DT_rlm$US_Origin)
DT_rlm$Long_Description <- factor(DT_rlm$Long_Description)
DT_rlm$Male = as.character(DT_rlm$Male)
DT_rlm$Household_Income = as.character(DT_rlm$Household_Income)
DT_rlm$Cab_Preference = as.character(DT_rlm$Cab_Preference)
DT_rlm$English_as_primary = as.character(DT_rlm$English_as_primary)
DT_rlm$Drink_Frequency = as.character(DT_rlm$Drink_Frequency)
DT_rlm$Purchase_Frequency = as.character(DT_rlm$Purchase_Frequency)


rlm.format<-rlm(Guess_the_Cost~French_Purchasing_Page + German_Purchasing_Page + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency, DT_rlm)


rlm.origin<-rlm(Guess_the_Cost~US_Origin + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency,DT_rlm)

rlm.description<-rlm(Guess_the_Cost~Long_Description + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency, DT_rlm)



stargazer(rlm.format,rlm.origin,rlm.description,main="RLM Basic regressions with covariates", align=TRUE, type="text")
...
rlm.saturated<-rlm(Guess_the_Cost~ Track + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency, DT_rlm)

summary(rlm.saturated)
...
rlm.same<-rlm(Guess_the_Cost ~  same_format_origin + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency, DT_rlm)

rlm.same_indiv<-rlm(d$Guess_the_Cost ~ French_format_wine + English_format_wine + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency, DT_rlm)
...
```{r}
rlm.cv<-rlm(Guess_the_Cost~French_Purchasing_Page + German_Purchasing_Page + Male + Household_Income + English_as_primary  + Drink_Frequency + Cab_Preference + Purchase_Frequency,DT_rlm)

rlm.cv2 <- rlm(Guess_the_Cost~ US_Origin + Male + Household_Income + English_as_primary  + Drink_Frequency + Cab_Preference + Purchase_Frequency,DT_rlm)

rlm.cv3 <- rlm(Guess_the_Cost~ Long_Description + Male + Household_Income + English_as_primary  + Drink_Frequency + Cab_Preference + Purchase_Frequency,DT_rlm)
stargazer(rlm.cv,rlm.cv2, rlm.cv3, align=TRUE,type="text")
```
...
```{r}
DT_rlm$Track<-factor(DT_rlm$Track)
rlm.saturated<-rlm(Guess_the_Cost~ Track + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency,DT_rlm)
summary(rlm.saturated)
```
...
rlm.cv<-rlm(Guess_the_Cost~French_Purchasing_Page + German_Purchasing_Page,DT_rlm)

rlm.cv2 <- rlm(Guess_the_Cost~ US_Origin,DT_rlm)

rlm.cv3 <- rlm(Guess_the_Cost~ Long_Description, DT_rlm)
stargazer(rlm.cv,rlm.cv2, rlm.cv3, align=TRUE,type="text")
```
...
rlm.cv_format<-rlm(Guess_the_Cost~French_Purchasing_Page + German_Purchasing_Page + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency ,DT_rlm)
#stargazer(lm.basic,lm.cv, title= "Basic Regressions",type="latex")

#covariates, treatment origin
rlm.cv_origin<-rlm(Guess_the_Cost~US_Origin + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency ,DT_rlm)
#summary(lm.cv_origin)

#covariates, treatment description
rlm.cv_description<-rlm(Guess_the_Cost~Long_Description + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency ,DT_rlm)
#summary(lm.cv_description)

stargazer(rlm.cv_format,rlm.cv_origin,rlm.cv_description, title= "Basic Regressions",type="text", align=TRUE)
```
...
rlm.same_indiv<-rlm(Guess_the_Cost ~ French_format_wine + English_format_wine + Male + Household_Income + English_as_primary + Drink_Frequency + Cab_Preference + Purchase_Frequency,DT_rlm)
stargazer(rlm.same,rlm.same_indiv,align=TRUE,type="text")
```
...
#2. Pre-clean General summaries- already in writeup
```{r}
d2 = read.csv('With_Second_Launch.csv')
d2<-data.table(d2)
#Sort for second launch
d2<-d2[Milk_Cost!=""]
stargazer(d2,summary=TRUE,type="text")
```
## De-dupping & cleaning
```{r deduping}
library(data.table)
# Cleaning out incomplete responses before treament exposures
d2 <- d2[!is.na(d2$Track),]
#summary(d)
# Counting occurances of IP address and Geo-codes
d2$geo_code <- paste(d2$LocationLatitude, d2$LocationLongitude)
dt = data.table(d2)
dt[, `freq_geo` := .N, by = geo_code]
dt[, `freq_ip` := .N, by = IPAddress]
#class(as.data.frame(dt))
...
stargazer(d2,summary=TRUE,type="text")
```
###Methods for Milk factors:
```{r}
#Milk a Factor of wine. only apply to relative filter
d2$Milk_Multiplier<-d2[,Guess_the_Cost/Milk_Cost]
d2_Milkfilter_abs<-d2[Milk_Cost<10 & Milk_Cost >0]
stargazer(d2_Milkfilter_abs, summary = TRUE, type = "text")
par(mfrow=c(1,2))
hist(d2_Milkfilter_abs$Milk_Multiplier,breaks = c(0,1,5,10,15,20,25,30,35,40,45,50,55),main="Histogram of Wine/Milk cost",xlab="$Milk * X = $Wine")
plot(d2_Milkfilter_abs$Milk_Cost,d2_Milkfilter_abs$Guess_the_Cost,main="Wine cost vs Milk cost \nFiltered, Milk<$10")
```

```{r}
#code for multiple filtering methods- plz ignore
par(mfrow=c(2,2))
hist(d2$Guess_the_Cost,main="Wine cost\nNo filter")
hist(d2_Milkfilter_wine$Guess_the_Cost,main="Wine cost\n$Milk< $Wine")
hist(d2_Milkfilter_abs_100$Guess_the_Cost,main="Wine cost\nFiltered, Milk<$100")
hist(d2_Milkfilter_abs_10$Guess_the_Cost,main="Wine cost\nFiltered, Milk<$10")


par(mfrow=c(1,3))
plot(d2$Milk_Cost,d2$Guess_the_Cost,main="Wine cost vs Milk cost \nNo filter")
plot(d2_Milkfilter_abs$Milk_Cost,d2_Milkfilter$Guess_the_Cost,main="Wine cost vs Milk cost \nFiltered, Milk<$10")
plot(d2_Milkfilter_wine$Milk_Cost,d2_Milkfilter_wine$Guess_the_Cost,main="Wine cost vs Milk cost \nFiltered, Milk<Wine")
hist(d2_Milkfilter_abs$Milk_Multiplier,breaks = c(0,1,5,10,15,20,25,30,35,40,45,50,55),main="Histogram of Wine cost/Milk cost",xlab="$Milk * X = $Wine")

length(d2_Milkfilter_abs_10$Milk_Cost)
plot(d2$Milk_Cost,d2$Guess_the_Cost,main="Wine cost vs Milk cost \nNo filter",xlim = c(0,50))
hist(d2[Milk_Cost<100]$Milk_Cost)
length(d2_Milkfilter_abs_10)
```
...
###post cleaning summaries
```{r}
summary(d2$Guess_the_Cost[d2$French_Purchasing_Page == 1]) #French language only
summary(d2$Guess_the_Cost[d2$German_Purchasing_Page == 1]) #German language only
summary(d2$Guess_the_Cost[d2$German_Purchasing_Page == 0 & d2$French_Purchasing_Page == 0]) #English language only
...
##RLM interaction- never purchase HTE
```{r}

HTE_test<-function(d,feature){
  rlm.HTE_format<-rlm(Guess_the_Cost~ French_Purchasing_Page + German_Purchasing_Page+ feature + French_Purchasing_Page*feature + German_Purchasing_Page * feature,d)
rlm.HTE_origin<-rlm(Guess_the_Cost~  US_Origin +feature+ feature * US_Origin,d)
rlm.HTE_description<-rlm(Guess_the_Cost~ Long_Description+feature + feature * Long_Description,d)
feature_title<-string(feature)

return(stargazer(rlm.HTE_format,rlm.HTE_origin,rlm.HTE_description, align=TRUE,type="text", title = paste("HTE test:", string(feature))))
}
string(d$Never_purchased)
HTE_test(d,d$Never_purchased)
#non-significant HTEs
HTE_test(d,d$English_as_primary)
HTE_test(d,d$Cab_Preference)
HTE_test(d,d$Male)
...
stargazer(rlm.HTE_format,rlm.HTE_origin,rlm.HTE_description, align=TRUE,type="text", title = "RLM regression- Interaction of Never-purchase-wine")
...
```{r}
rlm.HTE_format_EPL<-rlm(Guess_the_Cost~ French_Purchasing_Page + German_Purchasing_Page + English_as_primary+French_Purchasing_Page * English_as_primary + German_Purchasing_Page * English_as_primary,d)
rlm.HTE_origin_EPL<-rlm(Guess_the_Cost~  US_Origin + English_as_primary + English_as_primary * US_Origin,d)
rlm.HTE_description_EPL<-rlm(Guess_the_Cost~ Long_Description + English_as_primary + English_as_primary * Long_Description,d)

stargazer(rlm.HTE_format_EPL,rlm.HTE_origin_EPL,rlm.HTE_description_EPL, align=TRUE,type="text", title = "RLM regression- Interaction of Never-purchase-wine")
```
