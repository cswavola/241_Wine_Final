...
We first load the data gathered in the survey. We first go through some general data processing: we need to factorize some of the answers from demographics questions, and turn some other results into binary (0,1) variables for easier handling.
...
##1. EDA and Cleaning
##Pre-cleaning
Now that we've loaded the data, we look at some description statistics to understand our general results, and identify any potential issues that requires cleaning, or additional processing.
...
After a general overview, we noticed the following things that require attention: 
...
3). As seen from the histogram above, we noticed that we have many large outliers for guess-the-cost. The large variation in the answer will hurt our ability to obtain a reasonable SD and find significant findings. We need to pay special attenion in our modeling phase later, to try to minize the effect of outliers.

We proceed with cleaning as mentioned above.
...
After the cleaning, we look at the descriptive statistics on our outcome and demographics variables again. Here we see that cleaning reduced our outlier problem significantly, although it still exist. 
...
We dive a bit deeper into the guess-the-cost results, and see whether our treatment effect is obvious through histograms.
...
Before we start our modeling, we wanted to first validate our experimental assumptions, including conducting a covariance balance check, a check on potential differential attrition, and a check for potential heterogeneous treatment effect.

### Demographic/Covariate Balance Check
We start with the covariate balance check. Here we use an f test to see whether our covariates has any prediction power on which treatment track a respondent is assigned to.
...
d<-data.table(complete_dedup)
```
Based on the table above, we conclude that our data passes covariance balance check. No covariates has 1% p value in predicting any of the treatment variable, and the three 5-10% significant covariate is likely due to chance, given the large number of covariates that we are including. Since our randomization is done through a randomizer in the qualtrics platform, we also trust that our randomization is successful.

### Attrition check

As mentioned in the begining, our attrition is rather low for the entire study (~5%), so we are not very concerned about potential bias caused by potential differential attrition. However, we decided to do a check for completeness. Here, we use both linear regression and logistic regression (since outcome variable is binary) to check whether being assigned to certain treamtent leads to more or less attrition than others.
```{r} 
OR = function(regr){
...
For the regression tables below, we see that both linear and logistic regression suggest that seeing French or German purchasing page made people more likely to drop off. In other words, we do find evidence for differential attrition. However, since our attrition rate is very low, even if the 5% that dropped out have very extreme outcomes, they are unlikely to bias our estimate too significantly. Therefore, we are not very concerned about this effect.
...
Our first attempt to isolate a treatment effect using random inference andthe sharp null hypothesis. By nature of RI, in which a high outlier may swing the entire treatment- or control-assigned group, we limited the guesses to the bottom 95%. Using the reduced outcome set, we tested the simple treatments and our specified interaction (same format*origin) hypotheses. 
```{r echo=False}
...
```
Sadly, we saw no significant results. On to linear regression.
...
```
##Move to RLM
###Insert image of table from presentation
####Explanation of RLM>LM
...
stargazer(rlm.cv, rlm.cv1, rlm.cv2, rlm.cv3, align=TRUE,type="text", title = "RLM Basic regressions- with covariates (factors omitted from table)",omit=Factors_omit)
```
